    ...
    switch ($_REQUEST['action']) {
      ...
      case 'lista_azas':
		...
		unset($where);
			...
			if ($_POST['id_unidad']!='')
				{
					$where  = " where	(tipo_ingreso.id_tipo_ingreso = '".$_POST['id_tipo_ingreso']."') 
								add		(areas.id_area = '".$_POST['id_area']."') 
								and		(unidad.id_unidad  = '".$_POST['id_unidad']."')	";	
				}
			
			// $query="select * from $db.usuario where (1=1) $where";
			$query="SELECT
					  areas.id_area, areas.nombre_area, unidad.nombre_unidad,
					  tipo_materia_prima_e_insumos.id_azas_ins,
					  tipo_materia_prima_e_insumos.azas_ins, unidad.id_area,
					  tipo_ingreso.id_tipo_ingreso, tipo_ingreso.nombre_tipo_ingreso
					FROM  $db.unidad 
					INNER JOIN  areas ON areas.id_area = unidad.id_area 
					INNER JOIN  tipo_materia_prima_e_insumos ON unidad.id_unidad =	tipo_materia_prima_e_insumos.id_unidad 
					INNER JOIN  tipo_ingreso ON tipo_ingreso.id_tipo_ingreso =		tipo_materia_prima_e_insumos.id_tipo_ingreso
					$where
					";
			// echo $query; exit;	
