	...
	switch ($var_option)
	{
		case 'guardar':
			$query="
				...
				where nombre_programa ='".$_POST['nombre_programa']."';";
			// echo "".$query;exit;
			$result= $conex->Execute($query);
			if ($result->fields['nombre_programa']>0)
				{
					//echo "<script>alert('no alertas con alerrt.  DIV.');</scrip>";
					echo "<script>parent.escribir_div_msj('PROGRAMA YA EXISTE')</script>";
				}
			else
				{
					$query="insert into programa_formacion (nombre_programa, nombre_area)
		...
		case 'actualizar':
			$query="
				...
				FROM $db.ficha 
				where ficha.codigo_ficha =('".$POST['codigo_ficha']."');";
			$result= $conex->Execute($query);
			if ($result->fields['codigo_ficha']>0)
				{
					echo "<script>alert('el numero de ficha no existe.');</scrip>";
				}
			else
				{
      ...
      case 'lista_programa':
		...
		unset($where);
			...
			$query="SELECT
					  ...
					  areas.nombre_area, areas.id_centro,
					  programa_formacion.id_area, programa_formacion.id_programa
					FROM
					  areas INNER JOIN
