	...
	switch ($var_option)
	{
		case 'guardar':
			...
			if ($result->fields['n_documento']>0)
				{
					echo "<script>parent.escribir_div_msj('el numero de identificacion ya existe.')</script>";
				}
			else
				{
					$query="insert into registro_usuario (n_documento, nombres, apellidos, celular, clave, correo_usuario, id_ficha, id_estado, id_tipo_documento, id_rol)
					values('".$_POST[n_documento]."', '".$_POST[nombres]."','".$_POST[apellidos]."','".$_POST[celular]."','".$_POST[clave]."','".$_POST[correo_usuario]."','".$_POST[codigo_ficha]."','".$_POST[nombre_estado]."','".$_POST[nombre_tipo_documento]."','".$_POST[nombre_rol]."');
					";
    ...
    switch ($_REQUEST['action']) {
      ...
      case 'lista_usuario':
		...
		unset($where);
			...
			if ($_POST['id_rol']!='')
				{
					$where  = " where (rol.id_rol = '".$_POST['id_rol']."') ";	
				}
			// $query="select * from $db.usuario where (1=1) $where";
			$query=" SELECT	    rol.nombre_rol, registro_usuario.id_rol, registro_usuario.nombres,
								registro_usuario.apellidos, registro_usuario.n_documento
					 FROM  		$db.registro_usuario
