    ...
    switch ($_REQUEST['action']) {
      ...
      case 'lista_areas':
		...
		unset($where);
			...
			if ($_POST['id_centro']!='')
				{
					$where  = " where (regional.id_regional = '".$_POST['id_regional']."') 
								and   (centro.id_centro  = '".$_POST['id_centro']."')     ";	
				}
			
			// $query="select * from $db.usuario where (1=1) $where";
			$query="SELECT  regional.id_regional, regional.nombre_regional,
							centro.nombre_centro, areas.id_area, areas.nombre_area,
							areas.id_centro
					FROM	$db.centro 
					INNER JOIN  regional ON regional.id_regional = centro.id_regional 
					INNER JOIN  areas ON centro.id_centro = areas.id_centro 
					$where 
					";
			// echo $query; exit;	
