    ...
    switch ($_REQUEST['action']) {
      ...
      case 'lista_precios':
		...
		unset($where);
			...
			if ($_POST['id_precio']!='')
				{
					$where  = " where (lista_precios.id_precio = '".$_POST['id_precio']."') ";	
				}

			$query="SELECT
					  lista_precios.id_precio,
					  lista_precios.cantidad_unitaria,
					  lista_precios.precio_producto,
					  lista_precios.fecha_registro,
					  lista_precios.fecha_cierre,
					  productos.nombre_producto,
					  registro_usuario.n_documento,
					  unidad_de_medida.unidad_medida
					FROM
					  $db.lista_precios
					  INNER JOIN productos ON productos.id_producto =
					lista_precios.id_producto
					  INNER JOIN registro_usuario ON lista_precios.id_usuario =
					registro_usuario.id_usuario
					  INNER JOIN unidad_de_medida ON unidad_de_medida.id_unidad_medida =
