...
//******************** Fin Conexion a la DB ********************
  ...
  try {
    switch ($_REQUEST['action']) {
      ...
      case 'lista_autoconsumo':
		$jTableResult = array();
		unset($where);
			
			if ($_POST['id_autoconsumo']!='')
				{
					$where  = " where (autoconsumo.id_autoconsumo = '".$_POST['id_autoconsumo']."') ";	
				}
			
			// $query="select * from $db.usuario where (1=1) $where";
			$query="SELECT
		   autoconsumo . id_autoconsumo ,
		   autoconsumo . cantidad ,
		   autoconsumo . id_cantidad_producto ,
		   cantidad_productos . id_producto ,
		   productos . id_producto  AS  id_producto1 ,
		   productos . nombre_producto 
		FROM
		   autoconsumo 
		  INNER JOIN  cantidad_productos  ON  autoconsumo . id_cantidad_producto  =
			 cantidad_productos . id_cantidad_producto 
		  INNER JOIN  productos  ON  cantidad_productos . id_producto  =
