...
//******************** Fin Conexion a la DB ********************
  ...
  try {
    switch ($_REQUEST['action']) {
      ...
      case 'lista_regionales':
		$jTableResult = array();		
		unset($where);

			if ($_POST['id_depto']!='')
				{
					$where  = " where (municipio.id_depto = '".$_POST['id_depto']."') ";	
				}

			$query="SELECT  	municipio.id_municipio, municipio.nombre_municipio,
								municipio.id_depto, departamento.nombre_depto,
								regional.id_regional, regional.nombre_regional
					FROM		$db.departamento 
					INNER JOIN  municipio ON departamento.id_depto = municipio.id_depto 
					INNER JOIN  regional ON municipio.id_municipio = regional.id_municipio
					$where 
					";
			// echo $query; exit;
