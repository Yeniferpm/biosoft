...
<head>
  ...
  <script type="text/javascript">
    $(document).ready(function () {	
	   ...
	   $('#id_regional').change(function(){
			$.post("../controlador/ctrl_combos.php",{
				action       : 'buscar_autoconsumo',
				id_regional  : $('#id_autoconsumo').val()
			},function(data){
				$("#productos").html(data.autonsumo);
			},'json');
	   ...
	   $('#TableContainer').jtable({
        title: 'LISTA AUTOCONSUMO REGISTRADOS',	//Se cambia según el formualrio
        sorting: true,
        defaultSorting: 'productos ASC',	//cambio
        paging: true,
        ...
        actions: {
          ...
          listAction: '../controlador/ctrl_grilla_autoconsumo.php?action=lista_autoconsumo',
          // deleteAction: '../controlador/ctrl_grilla_areas.php?action=elimina_usuario'
        },

		toolbar: {
			items: [{
				tooltip: 	'Haga click aqui para agregar una nuevo autoconsumo',
				icon: 		'../../herramientas/jtable/themes/lightcolor/check.png',
				text: 		'Agregar Nuevo  Autconsumo',
				click: function () {
					opcion_usuario('registra_autoconsumo', '', '0', '');
				}
			}]
		},
		...
		fields: {
          ...
          nombre_producto: {
			...
			btnEditar: {
				...
				edit: false,
				create: false,
				display: function (autoconsumoData) {
					...
					$img.click(function () {
						opcion_usuario('actualizar_autoconsumo', autoconsumoData.record.id_autoconsumo, '0', '');
					});
					return $img;
				}
			}
      ...
      $('#LoadRecordsButton').click(function (e) {	
			...
			$('#TableContainer').jtable('load', {
				id_autoconsumo: $('#id_autoconsumo').val(),
	...
	function opcion_usuario(action, id_usu, tipo, id_entidad)
		{
			ventana1('frm_autoconsumo.php?action='+action+'&id_usu='+id_usu+'&tipo='+tipo+'&id_entidad='+id_entidad, '700', '360', 'Actualizar Datos de Area');
		}	
	...
	<div id='div_opciones' style="text-align:center;margin-left:auto; margin-right:auto;width:100%;" >
		<table  width='50%' align='center' >
			<tr><td colspan='3' >LISTA DE AUTOCONSUMO INGRESADAS</td></tr>
			<tr>
				<td >CANTIDAD</td>
				<td >APLICACION EN</td>
			</tr>
			<tr>
				<td>
					<select type="text"  align='left' id='id_autoconsumo' name='id_autoconsumo' class="box1"  title="<?php echo $title_centros; ?>" required="" style="width:545px;cursor:pointer;" />	
						<option value='0'>:.</option >
						<?php
							$query="select id_autoconsumo, cantidad from autoconsumo;";
							$result = $conex->Execute($query);
							while(!$result->EOF)
							{
								echo "<option value='".$result->fields['id_autoconsumo']."'>".utf8_encode($result->fields['cantidad'])."</option >";
								$result->MoveNext();
							}
						?>
					</select>
				...
				<td>
					<select type="text"  id='productos' name='productos'   title="<?php echo $title_centros; ?>" style="width:545px;cursor:pointer;"/>	
					</select>	
				</td>
			</tr>
			<tr>
