	...
	switch ($var_option)
	{
		case 'guardar':
			$query="
				SELECT id_ventas
				FROM $db.ventas
				where id_ventas ='".$POST['id_ventas']."';";
			$result= $conex->Execute($query);
			if ($result->fields['n_documento']>0)
				{
					echo "<script>parent.escribir_div_msj('el numero de identificacion ya existe.')</scrip>";
				}
			else
				{
      ...
      case 'lista_ventas':
		...
		unset($where);
			...
			if ($_POST['id_producto']!='')
				{
					$where  = " where (productos.id_producto = '".$_POST['id_producto']."')	";	
				}
